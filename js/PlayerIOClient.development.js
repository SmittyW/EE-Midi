...
PlayerIO = {
	...
	*/
	...
	authenticate: function (gameId, connectionId, authenticationArguments, playerInsightSegments, successCallback, errorCallback) {
		...
		channel.authenticate(gameId, connectionId, _pio.convertToKVArray(authenticationArguments), _pio.convertToSegmentArray(playerInsightSegments), "html5", _pio.convertToKVArray({ "os": "windows", "browser": "firefox" }), null, successCallback, errorCallback, function (result) {
				...
				switch (typeof (value)) {
					...
					case 'object': // date, object & array
						if (value.getTime && value.getMilliseconds) { // date
							result.push({ valuetype: _pio.ValueType.DateTime, datetime: (value.getTime() - value.getMilliseconds()) / 1000 })
						} else {
			...
			if (value != null) {
				...
				switch (typeof (value)) {
					...
					case 'object': // date, object & array
						if (value.getTime && value.getMilliseconds) { // date
							if (value + '' != valueOriginal + '') {
								prop.value = { valuetype: _pio.ValueType.DateTime, datetime: (value.getTime() - value.getMilliseconds()) / 1000 }
								changes.push(prop)
							}
						} else {
...
(function () {
		...
		function connect(createSocket) {
			...
			function tryNextEndpoint() {
				if (endpointStrings.length > 0) {
						...
						function (connected, reason) { // onConnect
							...
							if (waitingForJoinResult) {
								if (msg.type == "playerio.joinresult") {
									...
									_pio.handleError(originalStack, errorCallback, PlayerIO.error.GeneralError, "The expected inital messagetype is: playerio.joinresult, received: " + joinResult.getType())
								}
							} else {
					...
					_pio.handleError(originalStack, errorCallback, PlayerIO.error.GeneralError, "Could not establish a socket connection to any of the given endpoints for the room")
				}
			}
	...
	*/
	_pio.message = function (type) {
		...
		* @param arguments Entries to add. Valid types are Number, int, uint, String, Boolean and ByteArray
		...
		this.add = function () {
			...
			for (var i = 0; i < arguments.length; i++) {
				...
				switch (typeof (value)) {
					...
					case 'boolean': self.addBool(value); break;
					case 'number':
						var isFloatingPoint = value % 1 != 0;
						if (isFloatingPoint) {
		...
		*/
		this.addInt = function (value) {
			add(value >= -2147483648 && value <= 2147483647, value, entryType_Integer, "an integer (32bit)");
		}
		...
		*/
		this.addUInt = function (value) {
			add(value >= 0 && value <= 4294967295, value, entryType_UnsignedInteger, "an unsigned integer (32bit)");
		}

		/** Add a value encoded as an long to the message
		* @param {number} value The number to add
		*/
		this.addLong = function (value) {
			add(value >= -9223372036854775808 && value <= 9223372036854775807, value, entryType_Long, "a long (64bit)");
		}

		/** Add a value encoded as an ulong to the message
		* @param {number} value The number to add
		*/
		this.addULong = function (value) {
			add(value >= 0 && value <= 18446744073709551615, value, entryType_UnsignedLong, "an unsigned long (64bit)");
		}

		/** Add a boolean value to the message
		* @param {bool} value The bool to add
		*/
		this.addBool = function (value) {
			add(true, value ? true : false, entryType_Boolean, "a boolean value");
		}

		/** Add a value encoded as an float to the message
		* @param {number} value The number to add
		*/
		this.addFloat = function (value) {
			
			add(true, value, entryType_Float, "a floating point value (32bit)");
		}

		/** Add a value encoded as an double to the message
		* @param {number} value The number to add
		*/
		this.addDouble = function (value) {
			
			add(true, value, entryType_Double, "a double floating point value (64bit)");
		}
		...
		* @return string */
		...
		function add(check, value, type, errorMessage) {
			if (check) {
				...
				throw _pio.error("The given value is not " + errorMessage);
			}
		}
	...
	*/
			...
			for (var i = 0; i != message.length; i++) {
				...
				switch (message._internal_('get-types')[i]) {
					...
					case entryType_UnsignedLong:
						writer.wrietLongPattern(shortUnsignedLongBottomPattern, unsignedLongBottomPattern, _pio.binaryserializer.bytesFromULong(value));
						break;
			...
			while (position < end) {
				...
				switch (pattern) {
					...
					case byteArrayBottomPattern:
						...
						if (position + length > end) {
							//_savedBytes = [[NSData dataWithBytes:bytes+startPosition length:end-startPosition] retain];            // save bytes for next recv
							//return;
				...
				switch (pattern) {
					...
					case stringTopPattern:
						...
						break;
					case integerBottomPattern:
						value = _pio.binaryserializer.intFromBytes(bytes, position, length); // [MessageSerializer intFromBytes:bytes position:position length:length];
					case integerTopPattern:
						if (partsInMessage == 0) {
							//If partsInMessage is 0, then we've justed started deserializing a new message, which means that
				...
				if (output != null && (--partsInMessage) == 0) {
					return output;
					/*[_parsedMessages addObject:output];
					[output release];
					_message = nil;*/
				}
			}
	...
	_pio.binaryserializer = {
		_intEncode: function (value, bytes, signed) {
		...
		bytesFromInt: function (value) {
			return this._intEncode(value, 4, true);
		},
		bytesFromUInt: function (value) {
			return this._intEncode(value, 4, false);
		},
		bytesFromLong: function (value) {
			return this._intEncode(value, 8, true);
		},
		bytesFromULong: function (value) {
			return this._intEncode(value, 8, false);
		},
		bytesFromFloat: function (value) {
			return new BinaryParser(true, true).encodeFloat(value, 23, 8)
		},
		bytesFromDouble: function (value) {
			return new BinaryParser(true, true).encodeFloat(value, 52, 11)
		},
		//------------

		powers: [Math.pow(256, 0), Math.pow(256, 1), Math.pow(256, 2), Math.pow(256, 3), Math.pow(256, 4), Math.pow(256, 5), Math.pow(256, 6), Math.pow(256, 7)],
		_intDecode: function (bytes, position, length, typeBytes, signed) {
			var offset = position + length - 1;
			var max = this.powers[typeBytes];
			var value = 0;
			for (var i = 0; i < length; i++) {
				value = value + bytes[offset - i] * this.powers[i];
			}
			return signed && value >= max / 2 ? value - max : value;
		},
		...
		floatFromBytes: function (bytes, position, length) {
			...
			//var v = jDataView.createBuffer.call(b);
			return decodeFloat(b, 1, 8, 23, -126, 127, true);
			//return this.readFloat32(b, 0);
			//return new BinaryParser(true, true).decodeFloat(b, 23, 8)
		},
		readFloat32: function (data, offset) {
			...
			console.log("Sign:" + sign + ", exp:" + exp + ", sig: " + sig)
			if (sig == 0 && exp == -127)
				return 0.0;
	...
	function decodeFloat(bytes, signBits, exponentBits, fractionBits, eMin, eMax, littleEndian) {
		...
		while (i < significandBin.length) {
			...
			val = val / 2;
			//console.log(significandBin.charAt(i) + ":::" + i + "..." + val)
			i++;
		}
